	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Stock Management System', 'height=1000,width=1800');
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select class="form-control" name="productName[]" id="productName'+count+'"  onchange="getProductData('+count+')" >'+
						...
						$.each(response, function(index, value) {
							...
							//alert(JSON.stringify(value[product_id]));
							
							value1 = capitalize_Words(value['product_name']);
							
							tr += '<option value="'+value['product_id']+'" id="changeProduct'+value['product_id']+'">'+value1+'</option>';	
					...
					tr += '</select>'+
					'<script type="text/javascript">$("#productName'+count+'").chosen();</script></div>'+
				'</td>'+
				...
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" onclick="getTotal('+count+')" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
